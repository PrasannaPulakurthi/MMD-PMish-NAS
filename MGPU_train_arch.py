...
def main():
    ...
    for epoch in tqdm(range(int(start_epoch), int(args.max_epoch_D)), desc='total progress'):
        ...
        if epoch % args.val_freq == 0 or epoch == int(args.max_epoch_D)-1:
            ...
            if args.trainprocedure == 'linear':
                # bu 4 -> 1024: epoch 0 -> 300
                bu_start = 4
                bu_end = args.bu_end
                N = int(args.max_epoch_D)
                steps = N/args.val_freq
                scale = (bu_end/bu_start) ** (1/steps)
                args.bu = scale ** (steps*epoch/N) * bu_start
                
            elif args.trainprocedure == 'saturate_linear':
            ...
            elif args.trainprocedure == 'saturate':
                # If there is no improvement for 30 epoches then load the best model
                if icounter == 0:
                    print(f'Upper bound changed from {args.bu} to {args.bu*2}.')
                    args.bu = args.bu * args.buincrate
